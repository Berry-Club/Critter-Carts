...
import dev.aaronhowser.mods.critter_carts.menu.ToggleSpriteButton
...
class ItemFilterScreen(
	...
	companion object {
		val BACKGROUND = ScreenBackground(CritterCarts.modResource("textures/gui/item_filter.png"), 176, 241)

		val INVERT_OFF = ScreenSprite(CritterCarts.modResource("filter/invert_off"), 16, 16)
		val INVERT_ON = ScreenSprite(CritterCarts.modResource("filter/invert_on"), 16, 16)
		val USE_TAGS_OFF = ScreenSprite(CritterCarts.modResource("filter/use_tags_off"), 16, 16)
		val USE_TAGS_ON = ScreenSprite(CritterCarts.modResource("filter/use_tags_on"), 16, 16)
		val IGNORE_DAMAGE_OFF = ScreenSprite(CritterCarts.modResource("filter/ignore_damage_off"), 16, 16)
		val IGNORE_DAMAGE_ON = ScreenSprite(CritterCarts.modResource("filter/ignore_damage_on"), 16, 16)
		val IGNORE_ALL_COMPONENTS_OFF = ScreenSprite(
			CritterCarts.modResource("filter/ignore_all_components_off"),
			16,
			16
		)
		val IGNORE_ALL_COMPONENTS_ON = ScreenSprite(
			CritterCarts.modResource("filter/ignore_all_components_on"),
			16,
			16
		)
	}
